...
namespace BackgroundRemovalMVP.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> SendRegisterCode([FromBody] UserRegisterRequest request)
        {
            ...
            {
                Console.WriteLine($"[FALLBACK] Gửi email lỗi, cấp mã ngẫu nhiên tạm thời {code}. Chi tiết: {ex.Message}");
                return Ok(new { message = $"Lỗi gửi email: {ex.Message}. Mã xác minh tạm thời là: {code}" });
            }
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordRequest request)
        {
            ...
            {
                Console.WriteLine($"[FALLBACK] Gửi email lỗi, cấp mã ngẫu nhiên tạm thời {otp}. Chi tiết: {ex.Message}");
                return Ok(new { message = $"Lỗi gửi email: {ex.Message}. Mã khôi phục tạm thời là: {otp}" });
            }
